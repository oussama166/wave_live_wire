<div class="content w-full p-5 space-y-4 bg-white rounded-lg font-Mulish">
    ...
    <div class="w-full">
        <livewire:utils.drop-down-search
            label="Search user"
            :labelOn="true"
        />
    </div>
    <form class="flex justify-between flex-wrap flex-grow flex-shrink-0 w-full gap-5"
          wire:submit.prevent="createVacation"
          wire:loading.class="bg-gray-200">
        @if($selectedValue)
            <x-form-input name="name" id="name" title="Name" placeholder="Insert your first name"
                          input-type="text" value="{{ $selectedValue->name }}" :label-on="true"
            ...
            <x-form-input name="role" id="role" title="Role" placeholder="Select you role" input-type="text"
                          value="{{ $selectedValue->role }}" :label-on="true"
            ...
            <x-form-input name="email" id="email" title="Email" placeholder="insert you email" input-type="text"
                          value="{{ $selectedValue->email }}" :label-on="true"
                          form-style="flex-shrink-0 max-w-lg w-full "
                          :set-disable="true"/>
        @endif
        ...
        <section class="flex items-center justify-center w-full">
            <x-form-button type="submit" value="Create Vaction" custom-class="max-w-sm w-[300px]" wire:loading.remove/>
        </section>
    </form>
