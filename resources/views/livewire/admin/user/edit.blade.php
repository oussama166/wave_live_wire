<div x-data="{
    ...
    <div x-ref="tabButtons"
        ...
        <button data-tab-id="3" @click="tabButtonClicked($el);" type="button"
            class="relative z-20 inline-flex items-center justify-center w-full h-10 px-3 text-sm font-medium transition-all rounded-md cursor-pointer whitespace-nowrap">
            Positions
        </button>
        <button data-tab-id="4" @click="tabButtonClicked($el);" type="button"
            class="relative z-20 inline-flex items-center justify-center w-full h-10 px-3 text-sm font-medium transition-all rounded-md cursor-pointer whitespace-nowrap">
            Contracts
        </button>
        <div x-ref="tabMarker"
    ...
    <div class="relative w-full mt-2">
        ...
        <div data-tab-id="3" x-show="tabContentActive($el)" class="relative w-full" x-cloak>
            <!-- Tab Content 3 -->
            <div class="overflow-hidden border shadow-sm rounded-xl bg-card text-neutral-900">
                <form method="post" class='flex-row flex-wrap w-full gap-5 bg-white'
                    wire:submit.prevent="changeSalaryInformation">
                    ...
                    <section class="flex flex-wrap items-start justify-between w-full gap-5 p-8 bg-white font-Inter">
                        <input type="hidden" id="id" name="id" value="{{ $user['id'] }}" />
                        <livewire:utils.dropdown label="Position" :data="$this->getPosition"
                            selectedItem="{{ $user['position_id'] }}" index="id" select-area="formPosition.position"
        ...
        <div data-tab-id="4" x-show="tabContentActive($el)" class="relative w-full" x-cloak>
            <!-- Tab Content 4 -->
            <div class="overflow-hidden border shadow-sm rounded-xl bg-card text-neutral-900">
                <h1>Hello world</h1>
            </div>
