<div class="flex flex-row flex-wrap gap-5 content">
    <form method="post" wire:submit.prevent='addEmployer'>
        ...
        <section
            ...
            <x-form-input
                name="addForm.birth_date" id="addForm.birth_date" title="Birth date"
                placeholder="insert you birth date"
            ...
            <x-form-input
                name="addForm.hiring_date" id="addForm.hiring_date" title="Hiring date"
                placeholder="insert you hiring date"
                input-type="datePicker" :label-on="true"
                form-style="flex-shrink-0 max-w-lg w-full"
                :set-disable="false"
            />
            <livewire:utils.dropdown
                label="Role" :data="$this->getRole"
                index="label" select-area="addForm.role"
                :wire:key="$selectArea"
            />
            <x-form-input
                name="addForm.email" id="addForm.email" title="Email" placeholder="insert you email"
                input-type="text"
                :label-on="true" form-style="flex-shrink-0 max-w-lg w-full "
                :set-disable="false"/>

            <livewire:utils.dropdown
                label="Gender" :data="$this->getSex"
                index="label" select-area="addForm.sexe"
            ...
            <x-form-input name="addForm.salary" id="addForm.salary" title="Salary" placeholder="insert your salary"
                          ...
                          form-style="flex-shrink-0 max-w-lg w-full " :set-disable="false"/>
            <livewire:utils.dropdown
                label="Experience level" :data="$this->getExperienceLevels"
                index="label" select-area="addForm.experience_level"
                :wire:key="$this->selectArea"
            />
            <livewire:utils.dropdown
                label="Position" :data="$this->getPositions"
                index="label" select-area="addForm.position"
                :wire:key="$this->selectArea"
            />
            <livewire:utils.dropdown
                label="Nationality" :data="$this->getNationality"
                index="label" select-area="addForm.nationality"
                :wire:key="$this->selectArea"
            />
            <livewire:utils.dropdown
                label="Family status" :data="$this->getFamilyStatus"
                index="label" select-area="addForm.family_status"
                :wire:key="$this->selectArea"
            />
            <livewire:utils.dropdown
                label="Contracts" :data="$this->getContracts"
                index="label" select-area="addForm.contract"
                :wire:key="$this->selectArea"
            />
            <x-form-input name="addForm.balance" id="addForm.balance" title="Leave balance"
                          placeholder="Choice your experience level" input-type="text"
