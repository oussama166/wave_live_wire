    ...
    <section
        ...
        <aside class="max-w-[calc(100%-60%)] w-full rounded-lg overflow-hidden border border-wave-secondary/80">
            ...
            <ul class="flex-col w-full divide-y divide-wave-secondary/80">
                ...
                <li class="w-full py-5 text-center" id="{{$contract['id']}}">
                    <div class="relative w-full">
                        ...
                        <div
                            ...
                            <div class="w-6 h-6 text-primary-500"
                                @click="$dispatch('changeDataContract',{id:'{{$contract['id']}}'})">
                                <x-eos-mode-edit-o />
                            </div>
                            <hr class="w-6 h-0.5 bg-primary500/40 rotate-90" />
                            <div class="w-6 h-6 text-primary-500"
                                @click="$dispatch('deleteDataContract',{id:'{{$contract['id']}}'})">
        ...
        <aside class="w-max-[50%] w-full space-y-5">
            ...
            <div>
                <h1 class="text-2xl uppercase text-primary-500">New Contract Type or Modified the existing one</h1>
            </div>
            <form wire:submit="updateContractType" class="space-y-5">
                @csrf
                <x-form-input name="label" id="label" title="Name of the contract"
                    placeholder="Insert the name of the contract type" input-type="text" labelStyle="text-wave-500"
                    labelStyle="text-primary-700" formStyle="max-w-full w-full" :label-on="true"
                    form-style="flex-shrink-0 max-w-lg w-full " :set-disable="false" />

                <x-form-input name="description" id="description" title="Description"
                    placeholder="Insert small description about this contract type" labelStyle="text-primary-700"
