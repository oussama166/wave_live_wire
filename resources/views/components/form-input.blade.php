...
<div class="{{ $formStyle }} space-y-3">
    <div class="w-full space-y-3">
        @if ($labelOn)
        <label for="{{ $id }}" class="{{$labelStyle}} block mb-2 text-sm font-medium dark:text-white">{{ $title
            }}</label>
        @endif
        ...
        <div x-data="{ showPassword: false }" class="relative">
            ...
            @if(@$inputType == "password")
            <input wire:loading.attr="disabled" :type="showPassword ? 'text' : 'password'" id="{{ $id }}"
            ...
            <textarea wire:loading.attr="disabled" id="{{$id}}" name="{{$name}}" rows="4"
                placeholder="{{ $placeholder }}" {{ $setDisable? 'disabled' : '' }} {{ $setFocus ? 'autofocus' : '' }}
                {{ $isRequire ? 'required' : '' }} {{$type}}='{{$name}}' class="
            ...
            <input type="color"
                ...
                id="{{$id}}" name="{{$name}}" {{$type}}="{{$name}}" value="{{$value}}" title="{{$placeholder}}" />

            @else
            <input wire:loading.attr="disabled" type="{{ $inputType }}" id="{{ $id }}" name="{{ $name }}"
